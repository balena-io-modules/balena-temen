...
macro_rules! test_eval_eq {
    ($engine:expr, $expression:expr, $result:expr) => ({
        ...
        assert_eq!($engine.eval($expression, &position, &data, &mut context).unwrap(), $result);
    });
...
macro_rules! test_eval_as_bool_eq {
    ($engine:expr, $expression:expr, $result:expr) => ({
        ...
        assert_eq!($engine.eval_as_bool($expression, &position, &data, &mut context).unwrap(), $result);
    });
    ($expression:expr, $result:expr) => ({
        let engine = balena_temen::Engine::default();
...
macro_rules! test_eval_as_bool_err {
    ($engine:expr, $expression:expr) => ({
        ...
        assert!($engine.eval_as_bool($expression, &position, &data, &mut context).is_err());
    });
    ($expression:expr) => ({
        let engine = balena_temen::Engine::default();
...
macro_rules! test_eval_as_bool_ok {
    ($engine:expr, $expression:expr) => ({
        ...
        assert!($engine.eval_as_bool($expression, &position, &data, &mut context).is_ok());
    });
    ($expression:expr) => ({
        let engine = balena_temen::Engine::default();
...
macro_rules! test_parse_eq {
    ($e:expr, $r:expr) => {
        assert_eq!(($e.parse() as balena_temen::error::Result<balena_temen::ast::Expression>).unwrap(), $r);
    };
}
